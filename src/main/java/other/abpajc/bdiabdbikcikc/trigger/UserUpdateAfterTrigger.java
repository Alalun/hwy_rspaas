...
public class UserUpdateAfterTrigger implements ScriptTrigger {
	...
	public ScriptTriggerResult execute(ScriptTriggerParam scriptTriggerParam) throws ScriptBusinessException {
		...
		try {
//			String user = getUser(id.toString());
//			JSONObject userJson = JSONObject.fromObject(user);
			Object unionId = list.get(0).getAttribute("positionName");
			if (unionId != null) {
				paramMap.put("id", unionId);
				String result = update(paramMap);
				logger.debug(result);
			}
	...
	private String getUser(String id) throws IOException {
		RkhdHttpClient rkhdHttpClient = new RkhdHttpClient();
		RkhdHttpData rkhdHttpData = new RkhdHttpData();
		rkhdHttpData.setCallString("/data/v1/objects/user/info");
	...
	private String update(Map<String,Object> body) throws IOException {
		CommonHttpClient commHttpClient = new CommonHttpClient();

		CommonData rkhdHttpData = new CommonData();
		rkhdHttpData.setCallString("https://rp14.crmatmobile.com/api//v1/user/update?timestamp="
